...
UnityLight CreateLight (Interpolators i,float3 wNormal) {
	...
	//half megaAttenuation = tex2D(_ToonRamp,half2(attenuation,0));
	//light.color = _LightColor0.rgb * megaAttenuation;
...
float4 MyFragmentProgram (Interpolators i) : SV_TARGET {
    ...
    #else
    float3 albedo = texSample.rgb*_Tint;    
    #endif
	float3 specularTint;
	...
	grayContribution=tex2D(_ToonRamp,half2(grayContribution,0.5));
	...
	//return fixed4(BRDF_light * grayContribution,texSample.a);
	//half lightScale = max(BRDF_light.x,max(BRDF_light.y,BRDF_light.z));
	//half toon = tex2D(_ToonRamp, float2(lightScale,0));
	//return fixed4(BRDF_light*toon,1);
	//return fixed4(BRDF_light,1);
}
#endif